...
//UT_hash* arghash = NULL;

//KHASH_MAP_INIT_STR( stiphash, STIPREC );
//static khash_t( stiphash ) * st;
...
static int val_stip( char *instr, ARGUMENT * arg )
{
    ...
    if ( ( *ptr == '=' ) && ( *( ptr + 1 ) != '\0' ) ) {
        ...
        for ( i = 0; i < NUMSTIPS; i++ ) {
            //k = kh_put( stiphash, st, stips[i].name, &ret );
            assert( ret != -1 );
            //kh_value( st, k ) = stips[i];
        }

        // k = kh_get( stiphash, st, ptr );

        //if ( k != kh_end( st ) ) {
        //STIPREC sr = kh_val( st, k );
...
static void set_up_argtable( void )
{
    ...
    //harg = kh_init( arghash );

    //for ( i = 0; i < ARGTYPES; i++ ) {
    //k = kh_put( arghash, harg, args[i].name, &ret );
    //assert( ret != -1 );
    //kh_value( harg, k ) = args[i];
    //}
